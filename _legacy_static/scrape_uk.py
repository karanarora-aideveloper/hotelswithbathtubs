"""
UK Hotels Scraper — London, Edinburgh, Manchester
Scrapes Booking.com with Jacuzzi filter, downloads images to R2,
inserts directly into live hotels MongoDB collection.
Usage: python3 scrape_uk.py
"""
...
print(f"\n{'='*60}")
print(f"UK Hotels Scraper — London, Edinburgh, Manchester")
print(f"{'='*60}")
...
def goto_with_retry(driver, url, max_attempts=3, cooldown=45):
    for attempt in range(1, max_attempts + 1):
        driver.get(url)
        time.sleep(5)
        if not is_stub_page(driver):
            return True
        print(f"  Stub page attempt {attempt}/{max_attempts}, cooling {cooldown}s...")
        ...
        try:
            ...
            if hotel_name:
                ...
                hotels.append({
                    ...
                    "bookingUrl": link,
                    "image": img_path or "",
                    "amenities": ["Jacuzzi", "Private Bathtub"],
                    "description": description or f"Verified hotel with private in-room bathtub in {city}, {country}.",
                    "rating": rating,
                ...
                print(f"  [{i+1}] No name at {link[:70]}")
...
def save_to_mongo(all_hotels):
    ...
    print(f"\nDone: {inserted} new hotels inserted")
    client.close()
...
def main():
    ...
    try:
        for idx, (city, country) in enumerate(CITIES_TO_SCRAPE):
            ...
            print(f"\n{city}: {len(city_hotels)} hotels")
            if idx < len(CITIES_TO_SCRAPE) - 1:
                print("Pausing 15s between cities...")
                time.sleep(15)
    finally:
        driver.quit()

    print(f"\nTOTAL: {len(all_hotels)} UK hotels scraped")
    for h in all_hotels:
        print(f"  {h['name']} ({h['city']}) rating={h.get('rating')}")
