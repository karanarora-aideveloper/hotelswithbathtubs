...
async function fixAllHotelLinksFast() {
  await mongoose.connect(process.env.MONGODB_URI);
  const Hotel = mongoose.models.Hotel || mongoose.model('Hotel', new mongoose.Schema({}, { strict: false }));

  const hotels = await Hotel.find({}).lean();
  console.log(`Processing direct hotel links for ${hotels.length} hotels via bulkWrite...`);
  ...
  for (const hotel of hotels) {
    ...
    const citySlug = slugifyBooking(hotel.city || 'india');
  ...
  if (bulkOps.length > 0) {
    const res = await Hotel.bulkWrite(bulkOps);
    console.log(`\n🎉 bulkWrite complete: ${res.modifiedCount} hotels updated!`);
  }
...
fixAllHotelLinksFast().catch(console.error);
